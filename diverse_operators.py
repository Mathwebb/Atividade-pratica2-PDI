...
def color_region(image: Image, new_color: tuple, region_color: tuple) -> Image:
    image = image.copy()
    if len(new_color) == 4 and len(region_color) == 4:
        result = Image.new('RGBA', image.size)
        image = image.convert('RGBA')
    if len(new_color) == 3 and len(region_color) == 3:
        result = Image.new('RGB', image.size)
        image = image.convert('RGB')
    if len(new_color) == 1 and len(region_color) == 1:
    ...
    for line in range(image.size[0]):
        for column in range(image.size[1]):
            if original_image_matrix[line][column] == region_color:
                result.putpixel((line, column), new_color)
            else:
